...
menu_lines = [
    ...
    (130,  'GET K$ : PRINT K$;'),
    (135,  'IF K$="$" THEN PRINT CHR$(21): GOTO 10'),
    ...
    (1040, 'PRINT ""'),
    (1045, 'PRINT "  USE SHIFT+4/SHIFT+8 FOR 40/80 COLS"'),
    (1050, 'PRINT "  WHEN DONE WITH A PROGRAM,"'),
    (1060, 'PRINT "  TYPE:  -MENU"'),
    (1070, 'PRINT ""'),
...
def gen_browse_program(file_var, hdr_expr, init_lines):
    ...
    L.append((1040, 'NC=0'))
    # NP2 = records remaining in this section on this page (never read past section end)
    L.append((1045, f'NP2=NR-PG*{PAGE_SIZE}: IF NP2>{PAGE_SIZE} THEN NP2={PAGE_SIZE}'))
    L.append((1050, 'FOR I=1 TO NP2'))
    ...
    L.append((2440, 'HGR2'))
    L.append((2450, 'PRINT D$"BLOAD ";PY$;",A$4000"'))
    L.append((2460, 'GET K$'))
    ...
    # $=shift+4 → 40 columns (CHR$(21));  *=shift+8 → 80 columns (PR#3)
    L.append((9000, 'IF K$="$" THEN PRINT CHR$(21): RETURN'))
...
year_menu_lines_init.append((ln+45, 'IF K$="$" THEN PRINT CHR$(21): GOTO 10'))
...
topic_init.append((ln+37, 'IF K$="$" THEN PRINT CHR$(21): GOTO 10'))
...
n_topics = len(topics_seen)
max2 = n_topics - 10  # highest ones digit for two-digit topics (e.g. 14 topics → max2=4)
topic_init.append((ln+50, f'IF TI=1 THEN GET K2$: PRINT K2$: IF K2$>="0" AND K2$<="{max2}" THEN TI=10+VAL(K2$)'))
topic_init.append((ln+55, f'IF TI<1 OR TI>{n_topics} THEN GOTO {ln+30}'))
